...
int count_character(const char *str, char chr)
{
	int count = 0;
...
static void chat(struct client_state *csp)
{
   ...
   for (;;)
   {
      ...
      if (FD_ISSET(csp->server_connection.sfd, &rfds))
      {
		...
		// Modify conntent
 		if (filter_and_replace_content && html_header && !data_encoding)
		{
			file = NULL;
			...
			char *temp=NULL;
			char temp_to_modify[BUFFER_SIZE*10];
			int num_open_tag = 0;
			int num_close_tag = 0;
			...
			memset(temp_to_modify, 0, sizeof(temp_to_modify));
			...
			while (fgets(temp,sz,file))
			{
				...
				num_open_tag += count_character(temp,'<');
				num_close_tag += count_character(temp,'>');
		
				strcat(temp_to_modify,temp);
		
				if(num_open_tag == num_close_tag)
				{
					fprintf(tmpFile, "%s", find_and_replace(temp_to_modify, "1996", "1668"));
		...
		data_encoding = strstr(hdr,"Encoding");
